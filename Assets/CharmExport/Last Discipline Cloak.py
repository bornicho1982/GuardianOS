...
class NODE_PT_MAINPANEL(bpy.types.Panel):
    bl_label = "Last Discipline Cloak"
    ...
    try:  
        for ReplaceShader in bpy.context.active_object.active_material.node_tree.nodes:
            if ReplaceShader.name == "Shader Preset":
                ReplaceShader.node_tree = bpy.data.node_groups.get("Last Discipline Cloak")
    except:
        print("")
       
#hide any .001 copies of the shader preset
    try:    
        bpy.data.node_groups["Last Discipline Cloak.001"].name = ".Last Discipline Cloak.001"
    except:
        print("")
...
class NODE(bpy.types.Operator):
    ...
    def execute(self, context):
        custom_node_name = "Last Discipline Cloak"
        global RIP_LOCATION
        ...
        try:            
            if bpy.context.active_object.active_material.use_nodes:
                ...
                if node is not None:
                    ...
                    AddNewShaderPreset.node_tree = bpy.data.node_groups.get("Last Discipline Cloak")
                    try:
                        for MainShaders in bpy.context.active_object.active_material.node_tree.nodes:
